#!/bin/bash
# Copy the final checkpoint of each binmarker run to the canonical
# transfer location (/workspace-vast/$USER/exp/models/), ready to scp to the
# robot workstation for deployment.
#
# Usage: ./scripts/transfer_binmarker_checkpoints.sh
set -euo pipefail

CK=/workspace-vast/edwardosunny/tactile-openpi/checkpoints
DEST=/workspace-vast/edwardosunny/exp/models/tactile-binmarker
mkdir -p "$DEST"

for task in cube tube charger dishwasher; do
  cfg="pi05_xarm_${task}_binmarker_lora"
